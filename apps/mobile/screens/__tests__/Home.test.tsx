...
jest.mock('pursuit/components/Cards/InsightsCard', () => ({
  InsightsCard: ({ insightsData }: any) => (
    <text testID="insights-card">Insights: {insightsData.id}</text>
  ),
}));

jest.mock('pursuit/components/Cards/BucketCard', () => ({
  BucketCard: ({ name, emoji }: any) => (
    <text testID="bucket-card">{emoji} {name}</text>
  ),
}));

jest.mock('pursuit/components/Cards/EventsCard', () => ({
  EventsCard: ({ event }: any) => (
    <text testID="event-card">{event.title}</text>
  ),
}));

jest.mock('pursuit/components/Cards/BucketItemCard', () => ({
...
jest.mock('pursuit/components/Carousel/Carousel', () => ({
...
describe('Home Screen', () => {
  ...
  const mockHomeData = {
    getHome: {
      ...
      recommendations: [
        { id: '1', title: 'Beach Cleanup', date: '2023-10-15', location: 'Santa Monica' },
        { id: '2', title: 'Tech Conference', date: '2023-11-20', location: 'LA' },
      ],
      upcoming: [
        { 
          id: '1', 
